//! `rttp://` URI codec — validate, canonicalise, derive (RFC-002 §10).
//!
//! Authority boundaries:
//!   Grammar ......... RFC-002 §10.2 ABNF (sole authority; no syntax added here)
//!   Case discipline . §10.3 (lowercase US-ASCII only) + §10.5 (fail closed)
...
/// The only error type raised by this module. Fail closed (§10.5): no
...
pub struct ParsedUri {
    ...
    pub pillar: String,
    pub root: String,
    /// The intent verb; empty string = omitted (§10.4 default operation).
    pub action: String,
...
/// §10.2 token form: 1*( a-z / 0-9 / "-" ), no leading/trailing '-'.
    ...
    for ch in value.chars() {
        if ch.is_ascii_uppercase() {
            return Err(RttpUriError(format!(
                "{} contains uppercase '{}' - lowercase US-ASCII only (RFC-002 §10.3)",
                what, ch
            )));
        }
...
/// §10.2 action syntax test — an OPEN set: only the form is judged, never the
...
///
/// Deterministic, pure computation, DNS-free (§10.5), independent of `action`
...
pub fn derive_route_shard(canonical_authority: &str) -> Result<[u8; ROUTE_SHARD_BYTES], RttpUriError> {
    ...
    if canonical_authority != canonical_authority.to_lowercase() {
        return Err(RttpUriError(
            "canonical_authority must be lowercase (RFC-002 §10.3)".into(),
        ));
    }
...
/// intent, never normalises (§10.5 fail closed).
