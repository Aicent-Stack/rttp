...
[RFC-002 §10](https://rttp.com/RFC-002/) · v1.2.6 · zero dependencies · no build step
...
```console
$ npx @aicent/rttp
[PASS] all 25 checks passed (node, independent implementation)
```
...
US-ASCII (RFC-002 §10.3) and a case variant is not a spelling difference — it is
a *different string*. Admitting both would give one address two spellings, which
is the ambiguity §10.3 exists to remove.
...
| `isValidAction(value)` | §10.2 `action` form test. The verb set is **open**. |
...
| `action` | Empty string when omitted (RFC-002 §10.4 default operation) |
...
| `parseUri(uri)` | The independent RFC-002 §10.2 implementation → `{ authority, action, shardHex }` |
...
`src/conformance.mjs` is written from RFC-002 §10 alone and **must never import
...
codec, and the codec does not get to make that decision. (§10.2 sets no length
...
| | |
|:---|:---|
| ✅ **Validation** | Real, and specified (RFC-002 §10.2 / §10.3). |
| ✅ **Canonicalisation** | Real, and specified. |
| ✅ **ROUTE_SHARD derivation** | Real, and specified. |
| ✅ **Conformance vectors + independent replay** | Published and generated. |
| ❌ **Framing, as a library API** | `buildFrame` is exported from `./conformance`, where it exists to replay frame vectors — it is not offered as a general codec. |
| ❌ **Transport** | Not here. There is no `send()`. |
| ❌ **Routing service** | Not here. The shard is computed locally; delivering a frame to that hash is an operator's job. |
| ❌ **Sealing (§11)** | Separate artifacts. |
| ❌ **Browsers** | Node only — it uses `node:crypto`. The derivation is trivial to reproduce elsewhere, and the vectors let you prove you did. |
...
* **`rttp` URI scheme** — submitted to IANA under RFC 7595, ticket **#1459939**,
  Provisional, **under review**. It is **not yet registered** — as of 2026-09-17
