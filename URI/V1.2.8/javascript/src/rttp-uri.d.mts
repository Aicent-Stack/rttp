...
export interface RttpUri {
  ...
  pillar: string;
  root: string;
  /** Empty string when omitted (sec. 10.4 default operation). */
  action: string;
...
/** The URI does not conform. Always fail closed (sec. 10.5). */
...
/** sec. 10.2 `action` syntax test. Judges form only - the verb set is open. */
