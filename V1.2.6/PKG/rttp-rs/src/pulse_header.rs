//! `PulseHeader128` — RFC-002 §4.1 official 128-byte hardware-aligned frame
//! header, plus the v1.2.6 extension block (`0x66`–`0x7F`).
//!
//! Field layout (big-endian, per §4.1):
//! ```text
...
//!   0x34  u8    TTL_PULSE
//!   0x35  u8    PRIORITY
//!   0x36  u128  ROUTE_SHARD   SHA-256(authority)[0:16] — see SPEC §5
//!   0x46  32B   AID_ORIGIN    256-bit originator identity
//!   --- v1.2.6 extension (reserved area allocation, SPEC §2) ---
...
//!   0x69  16B   ACTION        §10.2 verb (lowercase ASCII, zero-padded)
...
pub fn encode_action(action: &str) -> Result<[u8; ACTION_MAX_LEN], PulseHeaderError> {
    ...
    if !rttp_uri::is_valid_action(action) {
        return Err(PulseHeaderError(format!(
            "invalid action token {:?} (RFC-002 §10.2: 1*( a-z / 0-9 / '-' ))",
            action
        )));
    }
...
/// emit apart from SPEC_REV=1 (spec §4 R6); empty action = §10.4 default.
pub fn build(
    sequence_id: u128,
    ttl: u8,
    priority: u8,
...
/// Build directly from an `rttp` URI — the mapping §10.4 promises:
...
/// of SPEC §4). `expected_aid_origin` optionally pins the origin.
pub fn verify(
    raw: &[u8],
