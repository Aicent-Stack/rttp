/**
 * @aicent/rttp - RFC-002 §10 `rttp` URI validation, canonicalisation and
 * ROUTE_SHARD derivation.
 *
 * This module is the reference implementation of the mapping RFC-002 §10.4
 * promises:
 *     "Dereferencing an `rttp` URI emits one pulse ... carrying `action` as the
 *      intent verb."
 * The specification previously had syntax (§10.2 ABNF) but no "URI -> frame
 ...
 *
 * Authority (important):
 *   * URI **syntax** - the sole authority is the RFC-002 §10.2 ABNF. This module
 *     adds no syntax.
 *   * This module does exactly two things: (1) validate and **canonicalise**
 *     per §10.2/§10.3; (2) derive ROUTE_SHARD.
 *
 * Design constraints (each is a requirement in the specification):
 *   * §10.1  `intent` is either 8 lowercase hex digits (a 32-bit routing hash)
 *            or a readable organ token.
 *   * §10.2  `action` is an **open set**: 1*( %x61-7A / DIGIT / "-" ).
 *   * §10.3  The canonical form is **lowercase US-ASCII**; there is no userinfo,
 *            port, query or fragment.
 *   * §10.5  **No DNS**: ROUTE_SHARD is pure computation - no registry, no
 *            resolver, no network.
 *
 * Case is **not** normalised. A case variant is not a spelling difference, it is
 * a different string, and it is rejected (§10.3). Nothing here is ever accepted
 * because a check could not be performed: this module fails closed.
 *
 * ROUTE_SHARD derivation (see spec §5): the first 16 bytes of
...
/** §10.2: `lowhex = %x30-39 / %x61-66`; 8 of them is a 32-bit routing hash. */
export const HASH_INTENT_LEN = 8;

/**
 * The capacity of the frame's ACTION field (`0x69`..`0x78`, one byte of
 * ACTION_LEN at `0x68`). **Not** a syntax rule: §10.2 places no length limit on
...
// §10.3: this scheme defines no userinfo / port / query / fragment
...
/**
 * The URI does not conform. **Always fail closed** (§10.5: no fallback, and no
 * `rttps`).
 */
...
/**
 * §10.2 token check, with the case rule of §10.3 kept separate so that an
...
function checkToken(value, what) {
  ...
  for (const ch of value) {
    ...
    if (ch !== ch.toLowerCase()) {
      // §10.3: the canonical form is lowercase. Uppercase is not a normalisable
      // spelling difference - it is illegal input.
      throw new RttpUriError(
        `${what} contains uppercase '${ch}' - lowercase US-ASCII only (RFC-002 §10.3)`);
    }
    throw new RttpUriError(
...
/**
 * §10.2 `action` syntax test: 1*( %x61-7A / DIGIT / "-" ).
 *
 ...
 *   * **Pure computation** - no DNS, no registry, no network (§10.5).
 *   * **One-way** - the authority cannot be recovered from the shard.
 *   * **Independent of `action`** - it routes *where*, never *what*, which is
 *     exactly why `action` must be a separate frame field (see spec §5.3).
 *
...
export function deriveRouteShard(canonicalAuthority) {
  ...
  if (canonicalAuthority !== canonicalAuthority.toLowerCase()) {
    throw new RttpUriError('canonical_authority must be lowercase (RFC-002 §10.3)');
  }
...
export function parse(uri) {
  ...
  if (slash !== -1 && action === '') {
    // §10.2: path = "/" action, action = 1*(...). A trailing slash with an empty
